...
class Embedding:
    ...
    def build(self):
        self._build(Params.glove_word, self._word2index, self._index2word, "_word_emb", Params.vocab_size,
                    Params.word_emb_size)
        self._build(Params.glove_char, self._char2index, self._index2char, "_char_emb", Params.char_size,
                    Params.char_emb_size)
...
def padding_char(ids, max_len):
    ...
    for i, word in enumerate(_ids):
        if len(word) > Params.max_word_len: _ids[i] = [
                                                          Embedding.UNKNOWN_ID] * Params.max_word_len  # If the word length can't convert to vector, we should mark it as an unknown word
    ...
    with open(path, "rb") as fp:
        for i, line in enumerate(fp):
            ...
            for _doc in _data["documents"]:
                ...
                passage_char_ids.append(
                    padding_char([_dict.get_char_id(list(pw)) for pw in passage], Params.max_passage_len))
                question_char_ids.append(
                    padding_char([_dict.get_char_id(list(qw)) for qw in question], Params.max_question_len))
                ...
                word_indices.append([1 if _doc["answer_spans"][0] <= i <= _doc["answer_spans"][1] else 1 for i in
                                     range(Params.max_passage_len)])
