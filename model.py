...
def log_total_params():
    total_parameters = sum(
        [reduce(lambda x, dim: x * dim.value, variable.get_shape(), 1) for variable in tf.trainable_variables()])
...
class V_NET(object):
    def __init__(self, word_emb=None, char_emb=None):
        ...
        with self.graph.as_default():
            ...
            self.word_embeddings = tf.get_variable("word_emb", shape=word_emb.shape,
                                                   initializer=tf.constant_initializer(word_emb, dtype=tf.float32),
                                                   trainable=False)
            self.char_embeddings = tf.get_variable("char_emb", shape=char_emb.shape,
                                                   initializer=tf.constant_initializer(char_emb, dtype=tf.float32),
                                                   trainable=False)
    ...
    def build_matching(self):
        ...
        self.passage_encoding = match_layer.match(self.passage_encoding, self.question_encoding, self.passage_word_len,
                                                  self.question_word_len)
        if Params.is_training:
    ...
    def build_boundary_loss(self):
        ...
        with tf.variable_scope("boundary_loss"):
            ...
            self.boundary_loss = -tf.reduce_mean(
                tf.reduce_mean(tf.reduce_sum(tf.log(self.point_logits + 1e-8) * self.indices_prob, 2), 1))
    ...
    def build_content_loss(self):
        with tf.variable_scope("content_loss"):
            ...
            self.words_prob = tf.reduce_max(tf.one_hot(self.words_indices, 10),
                                            1)  # how to implement ([[1, 4]]) => ([[0, 1, 1, 1, 1, 0, 0, ...]])

            self.content_prob = tf.nn.sigmoid(vars["w_1_c"] * tf.nn.relu(vars["w_2_c"] * self.point_logits))
            self.content_loss = -tf.reduce_mean(
                tf.reduce_mean(tf.reduce_sum(self.words_prob * tf.log(self.content_prob + 1e-8), 2), 1))
    ...
    def build_verfication_loss(self):
        ...
        with tf.variable("verfication_loss"):
            ...
            scores = tf.multiply(tf.transpose(self.answer_encoding),
                                 self.answer_encoding) * mask  # Warning: Judge if the answer_encoding from the same passage?

            self._answer_encoding = tf.matmul(self.answer_encoding, tf.nn.softmax(scores))
            g_v = tf.contrib.full_connected_layers(
                tf.concat([self.answer_encoding, self._answer_encoding, self.answer_encoding * self._answer_encoding],
                          2))

            p_v = tf.nn.softmax(g_v)
            self.cross_passage_verfication = -tf.reduce_mean(tf.log(p_v + 1e-8) * mask,
    ...
    def build_train(self):
        ...
        self.train_op = tf.train.AdamOptimizer(learning_rate=1e-3, beta1=0.9, beta2=0.999, epsilon=1e-8).minimize(
            self.loss)
...
def main(_):
    ...
    with model.graph.as_default():
        ...
        sv = tf.train.Supervisor(logdir=Params.logdir, save_model_secs=0, global_step=model.global_step,
            ...
            for epoch in range(1, Params.num_epochs + 1):
                ...
                for step in tqdm(range(model.num_batch), total=model.num_batch, ncols=70, leave=False, unit='b'):
                    ...
                    if step % Params.save_steps == 0:
                        ...
                        sv.saver.save(sess, Params.logdir + "/model_epoch_%d_step_%d" % (
                            gs / model.num_batch, gs % model.num_batch))
